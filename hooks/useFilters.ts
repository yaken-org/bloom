...
export interface UseFiltersReturn {
  ...
  // オプション操作
  setFilterOptions: (filterType: FilterType, options: Partial<FilterOptions>) => void;
...
export const useFilters = (
  ...
  // 現在の設定を取得（メモ化）
  const settings = useMemo((): FilterSettings => ({
  ...
  const setFilterEnabled = useCallback((filterType: FilterType, enabled: boolean) => {
  ...
  // 順序操作関数
  const reorderFilters = useCallback((newOrder: FilterType[]) => {
  ...
  // オプション操作関数
  const setFilterOptions = useCallback((filterType: FilterType, options: Partial<FilterOptions>) => {
  ...
  const getFilterOptions = useCallback((filterType: FilterType): FilterOptions => {
    return manager.getFilterOptions(filterType);
  }, [manager]);
  
  const updateFilterOption = useCallback(<K extends keyof FilterOptions>(
    filterType: FilterType,
    key: K,
    value: FilterOptions[K]
  ) => {
  ...
  const importSettings = useCallback((newSettings: Partial<FilterSettings>) => {
